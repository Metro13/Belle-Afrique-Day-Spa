<?php
        ...
        if (isset($_SESSION['product_id']) && isset($_SESSION['transaction-id']) && isset($_SESSION['transaction-amount']) && isset($_SESSION['auth_id'])) {

            $transId = $_SESSION['transaction-id'];
            $transAmount = $_SESSION['transaction-amount'];
            $userID = $_SESSION['auth_id'];
            $prodID = $_SESSION['product_id'];
            $updateOrders = $client->updateOrders($prodID, $userID);
            
        }

        $productData = $client->getSingleProduct($prodID);
...
<body>
    <div class="w-[600px] mx-auto text-gray-800 bg-gray-100">
        ...
        <div class="bg-white p-4">
            ...
            <p class="text-base p-2  bg-gray-50 font-normal tracking-tight"><?php echo $product->product_name; ?></p>
        </div>
        <div class="bg-white  p-4">
            <p class="text-lg text-indigo-600 font-medium">Description</p>
            <p class="text-base p-2  bg-gray-50 font-normal tracking-tight"><?php echo $product->description; ?></p>
        </div>
        <div class="bg-white p-4">
